...
OdometryServer::OdometryServer(const ros::NodeHandle& nh, const ros::NodeHandle& pnh)
    ...
    pnh_.param("output_file_path", output_file_path_,
               std::string("/home/wqf/Result/lo/123.txt"));
    ...
    if (publish_debug_clouds_) {
        ...
        kpoints_publisher_ = pnh_.advertise<sensor_msgs::PointCloud2>("/lo/keypoints", queue_size_);
        map_publisher_ = pnh_.advertise<sensor_msgs::PointCloud2>("/lo/local_map", queue_size_);
        global_map_publisher_ =
            pnh_.advertise<sensor_msgs::PointCloud2>("/lo/global_map", queue_size_);
    }
...
void OdometryServer::SavePoseToTUMFormat(const Sophus::SE3d& pose,
...
void OdometryServer::PrintStatistics() const {
    ...
    if (frame_counter_ > 0) {
        std::cout << "Average Wall time per frame: "
                  << (total_wall_time_ / frame_counter_) << " ms" << std::endl;
    }
}
