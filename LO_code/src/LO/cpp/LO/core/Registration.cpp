...
for (int j = 0; j < MAX_NUM_ITERATIONS_; ++j) {
    ...
    if (std::abs(dx(2)) > dz_gate) {
        Eigen::Vector6d dx_clamp = dx; dx_clamp(2) = std::copysign(dz_limit, dx(2));
        Eigen::Vector6d dx_zero  = dx; dx_zero(2)  = 0.0;
        double g_full  = predicted_gain(dx);
        double g_clamp = predicted_gain(dx_clamp);
        double g_zero  = predicted_gain(dx_zero);
        if (g_clamp >= g_full && g_clamp >= g_zero)      dx = dx_clamp;
        else if (g_zero >= g_full && g_zero >= g_clamp)  dx = dx_zero;
    }
