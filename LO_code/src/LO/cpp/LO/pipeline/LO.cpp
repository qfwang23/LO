...
//     // 调用地面点去除（用默认参数即可，如需调优可调整参数）
// const auto noground_frame = lo::RemoveGroundPointsAdvanced(
//  cropped_frame, 0.5, 0.2, 0.4, 15.0, 0.4);


    // auto [non_ground, ground] =MyGroundRemoval(cropped_frame, cfg);
...
std::vector<Eigen::Vector3d>  LO::Dedistortion(const std::vector<Eigen::Vector3d> &frame) {
